...
Les runes sont des mots-clés plutôt que des valeurs — elles ne peuvent pas être assignées à une
variable ou passées à une fonction, elles peuvent seulement être appelées. En référencer une sans
parenthèses est donc une erreur...

```js
let count = $state;
```

... que ce soit une rune comme `$state` ou une rune à laquelle on accède via une propriété, comme
`$derived.by`. Ajoutez les parenthèses, ainsi que les éventuels arguments que la rune attend :
...
Une assignation à une propriété de classe qui utilise une rune `$state` ou `$derived` est considérée
comme une _déclaration de propriété d'état_. La déclaration peut se produire dans le corps de la
classe...
...
... mais ne peut se produire qu'une seule fois.
...
Utiliser un préfixe `$` pour faire référence à la valeur d'un store n'est possible que dans les
fichiers `.svelte`, où Svelte peut automatiquement créer des abonnements lorsqu'un composant est
monté et se désabonner lorsque le composant est démonté. Envisagez plutôt la migration vers les
runes.
...
Voir https://html.spec.whatwg.org/multipage/custom-elements.html#valid-custom-element-name pour plus
d'informations sur les noms de balises valides.
