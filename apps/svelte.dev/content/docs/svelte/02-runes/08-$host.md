...
`$host` fournit un accès à l'élément hôte, vous permettant (par exemple) de générer des évènements personnalisés
Lorsque vous compilez un composant en tant que [élément personnalisé](custom-elements), la rune
